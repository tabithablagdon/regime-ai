...
Regime AI is a multi-agent decision-support platform engineered for buy-side research analysts, quantitative portfolio managers, and risk managers. Given a single U.S.-listed equity ticker, it produces an auditable forecast report - a probability distribution over bullish/neutral/bearish outcomes, a written thesis, and a full dissent log - by running two independent specialist agents in parallel and reconciling their claims through a deterministic synthesis pipeline, never a black box.

It is a decision-support tool only. **No code path in this system can place, size, or route a trade** as a hard architectural constraint, not a UI-level one, and every report says so.

This README covers what the system does, how the pieces fit together, what
each component is responsible for, and how to run it locally. 

## Table of contents

- [What it does](#what-it-does)
- [How to read a forecast report](#how-to-read-a-forecast-report)
...
## How to read a forecast report

Every report — whether you're looking at it in the web UI, the Markdown
file the CLI writes, or the JSON payload from the API — has the same six
parts, in the same order. Here's what each one means and how to weigh it,
using a real AAPL run as an example.

### 1. Header: recommendation and confidence

```
AAPL — 21-Day Horizon
Recommendation: Bullish Lean
Overall confidence: 67%
```

`recommendation` is one of four values, deliberately never "buy"/"sell":

| Value | Meaning |
|---|---|
| `bullish_lean` | The distribution leans bullish. |
| `neutral` | No meaningful lean either way. |
| `bearish_lean` | The distribution leans bearish. |
| `requires_review` | Confidence is low, or the two agents still disagree after the critique round — see "Escalation" below. Read this as "we don't have a clean read," not as a fifth directional signal. |

`overall_confidence` is the average of both agents' self-reported
confidence (post-critique, if a critique round ran) — not a statistical
p-value or a backtested win rate. Read it as "how sure the two agents are
in their own claims," not "probability the recommendation is correct."
...
These three numbers always sum to 100 and are computed with NumPy/SciPy —
a Monte Carlo mixture of each agent's claim, weighted by the
regime-conditioned weight table — never estimated or guessed by an LLM.
This is the report's actual quantitative output; everything else in the
report exists to explain and ground it.

### 3. Thesis

A short paragraph written last, after the distribution and every other
number is already fixed. The model's only job here is to narrate the
numbers above and summarize the two agents' reasoning in plain English —
never to introduce a new claim or restate a different figure. If the
thesis ever mentions a number that doesn't match the distribution or the
agent claims below it, that's a bug worth reporting, not a nuance to
reconcile.

### 4. Dissent log — both agents' raw claims

This is the report's most important section for deciding how much to
trust it. Both specialists' claims are always shown, unedited, side by
side — the blended distribution above never silently averages away
disagreement.

**Technicals** — a rule-based read of price/volume action:

| Field | Meaning |
|---|---|
| `direction` / `magnitude_bps` | Expected move over the horizon, in basis points (100 bps = 1%). |
| `confidence` | The agent's self-reported confidence, 0–1. |
| `regime_label` | One of `Trending Bull`, `Trending Bear`, `Mean-Reverting`, `Volatile/Choppy` — computed in code from ADX, realized volatility, and price vs. the 50-day moving average, never by the LLM. |
| `regime_changed` | Whether this differs from the ticker's last recorded regime. |

**Fundamentals/Sentiment** — grounded in the company's own disclosures and
recent news:

| Field | Meaning |
|---|---|
| `sentiment_score` | 1–100 read of recent news, 1 = very negative, 100 = very positive. |
| `key_risk_flags` | Short tags for anything concerning found in filings or news (e.g. `guidance_cut`). |

**Both agents also report:**

| Field | Meaning |
|---|---|
| `evidence` | What the claim is grounded in — a specific indicator value, a filing section + date, or an article + date. Check this before taking a claim at face value. |
| `falsifiers` | What observation would prove this specific claim wrong. A forcing function against unfalsifiable hand-waving — if you're deciding whether to trust a claim, check whether its falsifiers have already happened. |

**Critique exchange** (only shown if the two agents materially
disagreed — opposed directions, both confidence above 0.5): each agent
saw the other's claim and evidence and got one chance to revise. Both the
before and after versions are logged here; the dissent log above always
shows the *original* claims, never the revised ones, so you can see
exactly what changed and why.

### 5. Citations

Every filing/news source referenced anywhere in the report, deduplicated,
each with a date. This is what "auditable" means in practice — every
fundamental or sentiment claim should trace back to something in this
list.

### 6. Escalation

`escalate_to_analyst` (shown as "Requires Analyst Review") is `true`,
computed deterministically — never an LLM's judgment call — whenever:

- Overall confidence falls below a threshold (0.4 by default), or
- The two agents still materially disagree after the single critique
  round, or
- One of the two specialist agents failed entirely (single-agent mode).

When this flag is set, don't read the recommendation as a confident
directional call — it's the system telling you a human needs to look at
the dissent log and evidence directly before forming a view.

### The one line that's always there

Every report — successful or escalated — ends with the same footer,
unconditionally:

> Decision support only — not a trade recommendation. Requires human
...
On every run both, raw agent claims, the critique exchange if any, and the final synthesis, is persisted before the response returns, so any report can be reconstructed and audited after the fact.
...
[How it works](#how-it-works) above - see that section for the full five-step pipeline (validate → conflict-check → critique → fit → narrate).
...
Every agent and the Meta-Agent call through one interface - `LLMClient.complete_structured(system_prompt, user_prompt, response_model)`. Avoid using a OpenRouter or Anthropic SDK directly. `OpenRouterClient` is the
...
**Setup** — the backend must already be running first (`make up`, above):

```bash
cd web
npm install
cp .env.local.example .env.local   # defaults to http://127.0.0.1:8000; edit BACKEND_URL if the API runs elsewhere
npm run dev
```

Open `http://localhost:3000`. The page never calls the FastAPI backend
directly — it goes through a server-side proxy route
(`app/api/forecast/route.ts`), so the backend's address is never exposed
to the browser and no CORS configuration is needed.

**Using it:**

1. Enter a ticker symbol (e.g. `AAPL`) and a horizon in trading days
   (defaults to 21).
2. Click **Get forecast**. A run can take up to about a minute — both
   specialist agents run concurrently against real vendor APIs, then the
   Meta-Agent reconciles their claims.
3. One of three things happens:
   - **A rendered report** — the same content as the Markdown/CLI/API
     output, laid out for the browser. See
     [How to read a forecast report](#how-to-read-a-forecast-report)
     above for what each part means.
   - **"Ticker not found"**, shown inline next to the input — the ticker
     doesn't exist or isn't a supported U.S. equity.
   - **"Analysis engine unavailable"**, shown as a banner — the reasoning
     pipeline failed for any reason (most commonly, `OPENROUTER_API_KEY`,
     `FMP_API_KEY`, or `VOYAGE_API_KEY` in the backend's `.env` not being
     real keys yet, or a vendor rate limit), or the backend isn't
     reachable at all. This is expected until real vendor keys are
     configured — see [Configuration reference](#configuration-reference).

See `web/README.md` for the app's internal structure.
